...
await p.waitForFunction(()=>/good morning|plan a new trip/i.test(document.body?.innerText||''),{timeout:60000}).catch(()=>{});
await p.waitForTimeout(3000);
for (const [route,name] of [['/profile','profile'],['/trips','trips']]) {
  await p.goto('http://localhost:8081'+route, { waitUntil:'domcontentloaded', timeout:60000 });
  await p.waitForTimeout(6000);
  await p.screenshot({path:`../design/_shots/tab-${name}.png`});
}
await b.close();
